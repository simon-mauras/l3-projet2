    ...
    struct
      ...
      let solve out form =
        ...
        while !continue do
          ...
          if Formula.isFalse form then begin
            let rec unstack () =
              if Stack.is_empty stack
                then continue := false
                else match Stack.pop stack with
                  | Bet x -> Formula.forgetLiteral x form;
                             Formula.setLiteral (Literal.neg x) form;
                             Stack.push (Deduction (Literal.neg x)) stack
                  | Deduction x -> Formula.forgetLiteral x form;
                                   unstack() in
            unstack()
          end else if Formula.isTrue form then
            continue := false
          else begin
            ...
            if not !modif then
              (match Formula.getFreeLiteral form with
                ...
                | Some x -> Formula.setLiteral x form;
